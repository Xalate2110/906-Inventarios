...
//////////////////////////////////
		...
		//	$yy = $y->add();
			...
			$sell->compra_creditop = 0;

			if ($_POST["p_id"] == 4) {
				$sell->compra_creditop = 1;}
	            $s = $sell->add_re();
			...
			if($_POST["d_id"]==2){ $operation_type= 3; // 3.- entrada-pendiente 
	 		...
	 		if ($_POST["p_id"] == 4) {
				...
				$payment->sell_id = $s[1];
				$payment->val = round($_POST["total"],2);
...
print "<script>setTimeout(function(){ w = window.open('reporte_compra.php?id=$s[1]&sucursal=$sell->stock_to_id','ticket','height=720,width=1280');  w.print();   }, 100);  </script>";
		}
	}
}
