...
<form method="post" action="index.php?action=addtocot" id="addtocot<?php echo $p->id;?>">
			<select class="form-control form-control-sm" name="price" required>
				...
				<option value="<?php echo $p->price_out4; ?>">$ <?php echo $p->price_out4; ?></option>
			</select>
		...
		<script>
		$("#addtocot<?php echo $p->id;?>").on("submit",function(e){
		e.preventDefault();
			$.post("./?action=addtocot",$("#addtocot<?php echo $p->id;?>").serialize(),function(data){
				dx = null;
				$.get("./?action=cartofsellcot",null,function(data2){
					$("#cartofcot").html(data2);
					dx = data2;
				});
					$("#cartofcot").html(dx);
...
<script>
	//	e.preventDefault();
			$.post("./?action=addtocot","product_id=<?php echo $product->id; ?>&q=1&discount=0&price=<?php echo $product->price_out;?>&descripcion=<?php echo $product->name;?>",function(data){
				/* dx = null;
