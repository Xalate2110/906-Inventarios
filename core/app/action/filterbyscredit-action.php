...
if (isset($_SESSION["user_id"])) {
    if (Core::$user->kind == 10) {
        //print_r("no ADMIN");
        $products = SellData::getAllBySQL(" where user_id=" . Core::$user->id . " and operation_type_id=1 and p_id=4 and d_id = 1 and is_draft=0 and compra_creditop = 1  order by created_at desc ");
    } else if (Core::$user->kind == 3 || Core::$user->kind == 4) {
        $products = SellData::getAllBySQL(" where operation_type_id=1 and p_id=4 and d_id=1 and is_draft=0 and stock_to_id=" . Core::$user->stock_id . " and compra_creditop = 1 order by created_at desc");
    } else {
//print_r($_GET);
        //print_r("es ADMIN");
        $sql = "select * from sell ";
        $whereparams = array();
        $whereparams[] = " (operation_type_id=1  and p_id = 4 and compra_creditop = 1) ";
...
<tr>
<td> <h6  style="text-align: right;font-size:16px;color:blue">Total De Compras A Crédito $ : <?php echo number_format($totalc,2, '.', ',') ?></h6> </td>
</tr></div></div>
...
<?php if(isset($_SESSION["user_id"])):
  ...
  <a  class="btn btn-sm btn-danger" onclick="cancelacion(<?php echo $sell->id;?>,<?php echo $sucursal;?>,<?php echo $sell->total;?>,<?php echo $sell->getF()->id;?>,<?php echo $sell->person_id;?>,<?php echo $sell->getP()->id?>,<?php echo $sell->getD()->id?>,<?php echo $sucursal;?>);"><i  class="bi-trash"></i></a>
   ...
   <?php endif;?>
    ...
    <td  style="text-align: center;">
   <a target="_blank" href="reportes_pdf/reporte_abonos_creditop.php?id=<?php echo $sell->id ?>&cliente=<?php echo $client->id;?>&sucursal=<?php echo $sucursal;?>" class="btn btn-xs btn-prymari" ><i class="bi-ticket"></i></a>
   </td> 
    <td style="text-align: center">
    <a href="index.php?view=makepaymentp&id=<?php echo $client->id;?>" class="btn btn-secondary btn-sm">Realizar Pago</a>
    </td>
    <td style="text-align: center">
    <a href="index.php?view=paymenthistoryp&id=<?php echo $client->id;?>&compra=<?php echo $sell->id ?>" class="btn btn-secondary btn-sm">Abonos Relacionados</a>
    </td>
   </tr>
