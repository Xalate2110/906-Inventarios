...
if ($product->kind == 2 || $c["q"] <= $q || $c["q"] >= $q || $c["q"] > $q || $c["q"] < $q || $c["q"] = $q) {
				...
				$error = array("product_id"=>$c["product_id"],"message"=>"No hay suficiente cantidad de producto en inventario. 1");
...
if($product->kind==2||$_POST["q"]<=$q){
	...
	$error = array("product_id"=>$_POST["product_id"],"message"=>"No hay suficiente cantidad de producto en inventario. 2");
	$errors[count($errors)] = $error;
	$can=false;
}
...
//		echo ":P";
		$error = array("product_id"=>$c["product_id"],"message"=>"No hay suficiente cantidad de producto en inventario. 3");
