...
Listado de productos para ajustar
</div>
...
$q= OperationData::getQByStock($product->id,StockData::getPrincipal()->id);
	...
	<tr class="<?php if($q<=$product->inventary_min){ echo "danger"; }?>">
		...
		<td  style="text-align: center;">
		<form method="post"  id="addtoge<?php echo $product->id; ?>">
		<div class="row">
		<div class="col-md-3">
...
<div class="input-group">
  ...
  <input type="text" style="text-align: center;" class="form-control" name="price_out" placeholder="$ Precio Salida" value="<?php echo $product->price_out; ?>">
		...
		<input type="text" class="form-control" required name="q" id="re_q<?php echo $product->id; ?>" placeholder="Cantidad Ajustar">
</div>
		<div class="col-md-3">
...
<script>
		$("#addtoge<?php echo $product->id; ?>").on("submit",function(e){
		e.preventDefault();
			$.post("./?view=addtoge",$("#addtoge<?php echo $product->id; ?>").serialize(),function(data){
