...
if(count($_POST)>0){
	...
	$payment2->add_payment();
       ...
       if($pago == $total){
		...
		$sq5 = "update sell set cred_pagop = 1 where id ='" . $idventa . "'";
       ...
       if($payment2->id_formapago == '5' ){
		...
		if($pago == $total){
		Core::alert("Se ha liquidado el crédito del proveedor, correctame.");
        print "<script>window.location='index.php?view=creditos_liquidadosp';</script>";
		}else if ($pago < $total){
        Core::alert("Se ha realizo el abono, correctamente al crédito del proveedor.");	
		print "<script>window.location='index.php?view=byscredit';</script>";
		}
		}
		?>