...
if (isset($_SESSION["user_id"])) {
    if (Core::$user->kind == 10) {
        //print_r("no ADMIN");
        $products = SellData::getAllBySQL(" where user_id=" . Core::$user->id . " and operation_type_id=2 and p_id=1 and d_id = 1 and is_draft=0 and credito_liquidado = 1  order by fecha_pago desc ");
    } else if (Core::$user->kind == 2 || Core::$user->kind==4) {
        $products = SellData::getAllBySQL(" where operation_type_id=2 and p_id=1 and d_id=1 and is_draft=0 and credito_liquidado = 1 and stock_to_id=" . Core::$user->stock_id . " ");
    } else {
//print_r($_GET);
        //print_r("es ADMIN");
        ...
        $sql2 = $sql . " where " . implode(" and ", $whereparams) . " and stock_to_id=" . Core::$user->stock_id . " order by fecha_pago desc";
    ...
    $products = SellData::getAllBySQL(" where person_id=$_SESSION[client_id] and operation_type_id=2 and p_id=1 and d_id=1 and is_draft=0 and credito_liquidado = 1 order by fecha_pago desc");
}
...
if (count($products) > 0) {
   ...
   <div class="card box-primary">
    ...
    <tr>
   <td > <h6  style="text-align: right;font-size:20px;color:#12a14b">Total Créditos Liquidados $ : <?php echo number_format($total_liquidado,2, '.', ',') ?></h6> </td>
   </tr>
   </div>
   </div>
     ...
     <input type="hidden" class="form-control" id="stock_id" name = "stock_id" value="<?php echo $sell->getStockTo()->id;?>"> <!-- Se envia el id de la forma de pago -->  
                        ...
                        <td style="text-align: center"> <?php
                            if ($sell->person_id != null) {
                                ...
                                $rf = $c->regimen_fiscal;    
                                $si_rs = $c->tiene_rs;    
                                echo utf8_encode($c->name . " " . $c->lastname);
                            }
                            ?> </td>
                        <td style="text-align: center"> <?php
                        ...
                        <td style="text-align: center">
                            ...
                            <a   target="_blank" href="ticket.php?id=<?php echo $sell->id; ?>"  class="btn btn-xs btn-prymari" ><i class="bi-ticket" ></i></a>
                            ...
                            <?php
                            if (isset($_SESSION["user_id"]) || Core::$user->kind == 1):
                            ...
                            <!-- <a <?php echo $disabled2; ?> href="index.php?view=delsell&id=<?php echo $sell->id; ?>" class="btn btn-xs btn-danger"><i class="fa fa-trash"<?php echo $disabled ?>></i></a> -->

                            <?php endif; ?>
                          </td>
                              <td  style="text-align: center;">
                            <?php if (Core::$user->kind == 1 || Core::$user->kind == 3): ?>
                              <a  target="_blank" href="reportes_pdf/nota_cred_liq.php?id=<?php echo $sell->id ?>&cliente=<?php echo $idusuario;?>&sucursal=<?php echo $id_almacen;?>"  class="btn btn-xs btn-prymari" ><i class="bi-ticket"></i></a>
                             </td> 
                              <?php endif; ?></td>
                              <td style="text-align: center">
