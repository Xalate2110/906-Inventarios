...
$whereparams = array();
$whereparams[] = " (operation_type_id=11 and p_id=0 and d_id=0) ";
...
if(isset($_GET["start_at"]) && isset($_GET["finish_at"]) && $_GET["start_at"]!="" && $_GET["finish_at"]!=""){
 ...
 $sql2 = " where ".implode(" and ", $whereparams)." order by created_at ASC";
...
Listado Salidas Mercancía
</div>
<div class="box box-primary table-responsive">
<table class="table table-bordered table-hover table-responsive datatable">
	<thead>
		<th style="text-align: center;" >Id Salida</th>
		<th style="text-align: center;" >Almacen Salida</th>
...
?>
   ...
   $sell->getStockTo()->name ;} ?></td>     <td style="text-align: center"><?php echo $sell->created_at;
   ?>
   </td>     
  ...
  <td style="text-align: center">
  <a target="_blank" href="ajuste_inventario_faltantes.php?id=<?php echo $sell->id ?>&sucursal=<?php echo $sell->stock_to_id ?>" class="btn btn-sm btn-secondary"><i class='bi-ticket'></i></a> 
   </td>
  </tr>
...
function cancelar_compra() {
  if (confirm("¿Seguro que deseas eliminar  la Salida?, al realizarlo se recalculara el Inventario de la sucursal de Salida")) {
return true;
} else {
return false;
