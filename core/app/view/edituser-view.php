...
<div class="card">
  <div class="card-header">
    <span class="box-title">Formulario Edición De Usuarios</span>
...
<?php if($user->kind ==1 || $user->kind==2||$user->kind==3):?>
  <div class="form-group">
    <label for="inputEmail1" class="col-lg-2 control-label">Sucursal Asignada</label>
    <div class="col-md-6">
    <?php 
...
<div class="form-group">
   <label for="inputEmail1" class="col-lg-3 control-label">Perfil Actual Usuario</label>
     <div class="col-md-6">
      <?php
      $sql_perfil = "SELECT * FROM perfiles";
      //echo $sql;
      $resultSet = $mysqli->query($sql_perfil);
      ?>
                                       
                                       <select name="perfil" id = "perfil" class="form-control js-example-basic-single">
                                                <option selected>Perfiles Usuarios</option>
                                                <?php
                                                while ($fila = $resultSet->fetch_assoc()) {
                                                    if ($fila['id'] == $user->kind) {
                                                        $seleccion = "selected";
                                                    } else {
                                                        $seleccion = "";
                                                    }
                                                    echo "<option value='" . $fila['id'] . "'" . $seleccion . ">" . $fila['id_perfil'] . "-" . $fila['descripcion'] . "</option>";
                                                }
                                                ?>
                                            </select>
...
<div class="checkbox">
  ...
  <p class="text-info">* Campos obligatorios</p>
    <input type="hidden" name="user_id" value="<?php echo $user->id;?>">
      <button type="submit" class="btn btn-success" onclick="return procesar();">Actualizar Información Del Usuario</button>
    </div>
  </div>
</form>
</div>
...
<script>
        function procesar() {
        if (confirm("¿Deseas actualizar la información del usuario?")) {
        return true;
        } else {
        return false;}}
        </script>
