<section class="content"> 
<div class="row">
	<div class="col-md-12">

<?php 
if(isset($_SESSION["client_id"])):?>
		<h1><i class='glyphicon glyphicon-shopping-cart'></i> Mis Compras</h1>
<?php else:?>
<!--
<div class="btn-group pull-right">
  <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
    <i class="fa fa-download"></i> Descargar <span class="caret"></span>
  </button>
  <ul class="dropdown-menu" role="menu">
  	    <?php if(Core::$user->kind==1):?>
    <li><a href="report/sells-word.php">Word 2007 (.docx)</a></li>
    <li><a href="report/sells-xlsx.php">Excel 2007 (.xlsx)</a></li>
<?php endif; ?>
<li><a onclick="thePDF()" id="makepdf" class="">PDF (.pdf)</a></li>
  </ul>
</div>
-->

<div class="panel panel-default">
                        <div class="panel-heading">
						<h1><i class='glyphicon glyphicon glyphicon-shopping-cart'></i> Listado General Ventas A Créditos</h1>
						</div>
						 
                        </div>
<?php endif;?>
		<div class="clearfix"></div>

<form id="filtercreditosdia">
	<input type="hidden" name="view" value="sells">

<div class="row">
	
<?php if(Core::$user->kind==1):?>
	<div class="col-md-2">
		<label>Almacen</label>
		<select name="stock_id" class="form-control">
			<option value="">-- ALMACEN--</option>
			<?php foreach(StockData::getAll() as $stock):?>
				<option value="<?php echo $stock->id; ?>"><?php echo $stock->name; ?></option>
			<?php endforeach; ?>
		</select>
	</div>
	<?php endif; ?> 

	<div class="col-md-2">
		<label>Fecha inicio</label>
		<input type="date" name="start_at"  required class="form-control">
	</div>
	<div class="col-md-2">
		<label>Fecha fin</label>
		<input type="date" name="finish_at" required class="form-control">
	</div>
	<div class="col-md-2">
		<label>Aplicar Filtro</label><br>
		<input type="submit" value="Aplicar Filtro" class="btn btn-primary">
	</div>
</div>
</form>


<div class="allfiltercreditosdia"></div>


<script type="text/javascript">
	$(document).ready(function(){
		$.get("./?action=filtercreditosdia",$("#filtercreditosdia").serialize(),function(data){
			$(".allfiltercreditosdia").html(data);
		});

		$("#filtercreditosdia").submit(function(e){
			e.preventDefault();
		$.get("./?action=filtercreditosdia",$("#filtercreditosdia").serialize(),function(data){
			$(".allfiltercreditosdia").html(data);
		});

		})
	});
</script>
	</div>
</div>
</section>