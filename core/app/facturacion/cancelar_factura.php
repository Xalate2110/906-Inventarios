...
try {
    ...
    if ($numeroExcepcion == "0") {
        ...
        $sql_xml_up1 = "update sell set facturado = '0' where  invoice_code = '" . $folio . "';";
        $mysqli->query($sql_xml_up1);
    
        $sql_xml_up2 = "update cfdis set timbrado = '3'  where  UUID = '" . $folio . "';";
        $mysqli->query($sql_xml_up2);
    
        $sql_xml_up3 = "update cfdis set fecha_cancelacion = NOW()  where  UUID = '" . $folio . "';";
        $mysqli->query($sql_xml_up3);
        
        $sql_xml_up4 = "update cfdis set id_motcancelacion = '" . $motivoCancelacion . "'  where  UUID = '" . $folio . "';";
        $mysqli->query($sql_xml_up4);
        
        $sql_xml_up5 = "update cfdis set uuid_sustitucion = '" . $folioUUIDSustitucion . "' where  UUID = '" . $folio . "';";
        $mysqli->query($sql_xml_up5);
        
        $sql_xml_up6 = "update sell set invoice_code = ''  where  invoice_code = '" . $folio . "';";
        $mysqli->query($sql_xml_up6);
