<?php
    ...
    $sql_direccion1 = "SELECT * FROM razones_sociales where id = $id_almacen";
    $resultado1 = $mysqli->query($sql_direccion1);
    ...
    $direccion = $row['address1'];
       ...
       if($idcliente == '1' && $periodicidad == '0' && $p_ano == '0' && $p_mes == '0'){
        ...
        window.location = "https://localhost/?view=facturas_general&opt=all"; </script>';
    ...
    pruebaTimbrado($idcliente, $comprobante, $cfdi, $moneda, $fpago, $mpago, $email, $idcompra, $tipo_factura,$uuid_relacionado,$id_relacion,$id_almacen,$id_deposito,$comentarios,$usuario,$codigo_postal,$exportacion,$periodicidad,$p_mes,$p_ano,$folio_factura,$direccion);
    function pruebaTimbrado($idcliente, $comprobante, $cfdi, $moneda, $fpago, $mpago, $email, $idcompra, $tipo_factura,$uuid_relacionado,$id_relacion,$id_almacen,$id_deposito,$comentarios,$usuario,$codigo_postal,$exportacion,$periodicidad,$p_mes,$p_ano,$folio_factura,$direccion) {
    $rfc_emisor = "";

    $cfdi = generarXML($rfc_emisor, $idcliente, $comprobante, $cfdi, $moneda, $fpago, $mpago, $email, $idcompra, $tipo_factura,$uuid_relacionado,$id_relacion,$id_almacen,$id_deposito,$comentarios,$usuario,$codigo_postal,$exportacion,$periodicidad,$p_mes,$p_ano,$folio_factura,$direccion);
    ...
    window.location = "https://localhost/?view=facturas_general&opt=all";
    </script>
        <?php
     } 
}

function generarXML($rfc_emisor, $idcliente, $comprobante, $cfdi, $moneda, $fpago, $mpago, $email, $idcompra, $tipo_factura,$uuid_relacionado,$id_relacion,$id_almacen,$id_deposito,$comentarios,$usuario,$codigo_postal,$exportacion,$periodicidad,$p_mes,$p_ano,$folio_factura,$direccion) {
    ...
    $sql_conceptos = "SELECT operation.id,operation.product_id,operation.descripcion,operation.stock_id,operation.stock_destination_id,operation.operation_from_id,operation.q,operation.price_in,operation.price_out,operation.discount,operation.operation_type_id,
    ...
    cargaAtt($concepto, array(
        ...
        // si solamente concepto de impuesto es 02.
            ...
            $importeconceptos3 += round(($importeconceptos2) * 0.012500,6);

            /* Cálculos de Retenciones  IVA RETENIDO */
            $importeconceptos4 = ($fila_conceptos['q'] * $fila_conceptos['price_out']);
            $importeconceptos5 += round(($importeconceptos4) * 0.106666,6);

            $totalretenciones = round($importeconceptos3 + $importeconceptos5,6);
            ...
            cargaAtt($retencion, array(
                ...
                "TipoFactor" => "Tasa",
                "TasaOCuota" => "0.012500",
                "Importe" => round((($fila_conceptos['q'] * $fila_conceptos['price_out'])) * 0.012500,6)
                ));
    ...
    cargaAtt($root, array(
       ...
       if($longitud == '12'){
           ...
           /* impuestos retenidos */
           cargaAtt($impuestos1, array(
           "TotalImpuestosRetenidos" => round($totalretenciones,2))
           );
       }
    ...
    if (!file_exists($carpeta)) {
       ...
       if($fila> 0){
        echo "<script type='text/javascript'>
        alert('FOLIO DE FACTURA ENCONTRADO, SE PROCEDE A RETIMBRAR LA FACTURA ELECTRONICA');
        </script>";} 
    ...
    $sql_xml_sintimbrar = "UPDATE `cfdis` SET  folio='" . $foliox . "',serie='" . $fact_serie. "',regimen_fiscal='" . $rf. "',codigo_postal='" . $cp. "',periodicidad='".$periodicidad."',p_mes='" . $p_mes."',p_ano='" . $p_ano. "', rfc_receptor='" . $datoemisor . "',fecha_registro=NOW(),Folio_venta='" . $idcompra . "', tipo_factura='" . $tipo_factura . "',subtotal='" . $subtotal_total . "',iva='" . $suma_p2 . "',Monto='" . round($totalventa,2) . "',retenido='" . round($suma_p3,2) . "',uuid_relacionado='" . $uuid_relacionado . "',tipo='" . $id_relacion . "',mpago='" . $fpago . "',nombre_cliente='" . $name_cliente . "',apellido_cliente='" . $last_cliente . "',rfc_cliente='" . $no_cliente . "',email_cliente='" . $email . "',stock_id='" . $id_almacen . "',id_deposito='" . $id_deposito . "',comentarios='" . $comentarios . "',id_usuario='" . $usuario . "', id_cliente='" . $idcliente . "' WHERE folio_venta = '".$idcompra."'";
...
//echo $workspace;
    ...
    if ($xmlTimbrado != '') {
        ...
        $sql_xml_up4 = "UPDATE cfdis set error_timbrado  = 0 where folio = '" . $directorio . "';";
    ...
    foreach ($xml->xpath('//cfdi:Comprobante') as $cfdiComprobante) {
        ...
        foreach ($xml->xpath('//cfdi:Comprobante//cfdi:Complemento//tfd:TimbreFiscalDigital') as $tfd) {
            ...
            foreach ($xml->xpath('//cfdi:Comprobante//cfdi:Receptor') as $Receptor) {
                ...
                $pdf->Cell(19, 6, "CODIGO", 1, 0,"C", true);
                $pdf->Cell(12, 6, "CANT", 1, 0, "C", true);
                $pdf->Cell(19, 6, "UNIDAD", 1, 0, "C", true);
                $pdf->Cell(83, 6, "DESCRIPCION", 1, 0, "C", true);
                $pdf->Cell(25, 6, "VALOR UNITARIO", 1, 0, "l", true);
