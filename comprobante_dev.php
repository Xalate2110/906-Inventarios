...
$yy = 5; //Variable auxiliar para desplazarse 40 puntos del borde superior hacia abajo en la coordenada de las Y para evitar que el título este al nivel de la cabecera.
$y = $pdf->GetY(); 
$x = 12;
...
$pdf->Cell(60, -22, utf8_decode("COMPROBANTE DE DEVOLUCIÓN"), 0, 4, 'C');
...
$pdf->Cell(260,60,"Informacion General De Devolucion  _____________________________________________________________________");
...
$pdf->Cell(260,80, "Sucursal Venta : ". $stock->name);



$sql_venta="select * from sell where id = $id and operation_type_id = 5 ";
...
$iva = $row['iva'];     
$id_venta = $row['sell_from_id'];       

}

$pdf->SetFont('helvetica', '', 10); //Asignar la fuente, el estilo de la fuente (negrita) y el tamaño de la fuente
$pdf->SetXY(22, $y + $yy); //Ubicación según coordenadas X, Y. X=0 porque empezará desde el borde izquierdo de la página
$pdf->Cell(260,92, utf8_decode("ID Devolución : "). $id);


$pdf->SetFont('helvetica', '', 10); //Asignar la fuente, el estilo de la fuente (negrita) y el tamaño de la fuente
$pdf->SetXY(22, $y + $yy); //Ubicación según coordenadas X, Y. X=0 porque empezará desde el borde izquierdo de la página
$pdf->Cell(260,104, "ID Venta : ".$id_venta);
...
$pdf->Cell(260,80, utf8_decode("Fecha Devolución : ").$fecha_compra);


  $pdf->SetFont('helvetica', 'B', 10); //Asignar la fuente, el estilo de la fuente (negrita) y el tamaño de la fuente
  $pdf->SetXY(10, $y + $yy); //Ubicación según coordenadas X, Y. X=0 porque empezará desde el borde izquierdo de la página
  $pdf->Cell(260,135,utf8_decode("Listado De Productos En Devolución ____________________________________________________________________"));
